...
def about(request):
    aboutme = CompanyProfile.objects.get(pk=1)
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            newuser.pix = pix
            newuser.save()
            messages.success(request,f'congratulations {user.username} your registration is complete')
...
def profile_update(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            message.success(request, f'Dear {new}, your profile has been updated successfully')
...
def password_update(request):
    userprof = Customer.objects.get(user_username = request.user.username)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            message.error(request, f'Dear {new} your password change is not successful, {forms.errors}')
    ...
    if request.method == 'POST':
        ...
        if cart:
            ...
            if basket:
                ...
                basket.save()
                messages.success(request, format_html('one item added to cart ... <a href="http://44.201.172.113/cart">view cart</a>'))
                return redirect('home')
            else:
                newitem = Cart()
...
def pay(request):
    if request.method == 'POST':
        api_key = 'sk_test_2fd19a97824d54c70896ca6494d324b28297ea42' #secret key from paystack
        curl = 'https://api.paystack.co/transaction/initialize' #paystack call url 
        cburl = 'http://44.201.172.113/callback' #payment confirmation page 
